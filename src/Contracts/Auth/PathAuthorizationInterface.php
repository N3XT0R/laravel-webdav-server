...
interface PathAuthorizationInterface
{
     ...
     *
     * @param \N3XT0R\LaravelWebdavServer\ValueObjects\WebDavPrincipalValueObject $principal Authenticated principal requesting access.
     * @param string $disk Laravel filesystem disk that will be accessed.
     * @param string $path Resolved path on the target disk.
     *
     * @throws \Sabre\DAV\Exception\Forbidden When the principal is not allowed to read the path.
     */
     ...
     * @param string $path Resolved path on the target disk.
     *
     * @throws \Sabre\DAV\Exception\Forbidden When the principal is not allowed to modify the path.
     */
     ...
     * @param string $path Resolved path on the target disk.
     *
     * @throws \Sabre\DAV\Exception\Forbidden When the principal is not allowed to delete the path.
     */
     ...
     *
     * @throws \Sabre\DAV\Exception\Forbidden When the principal is not allowed to create the directory.
     */
     ...
     *
     * @throws \Sabre\DAV\Exception\Forbidden When the principal is not allowed to create the file.
     */
