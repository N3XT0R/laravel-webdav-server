...
- Legacy keys `webdav.storage.disk` and `webdav.storage.root` are still present in the config stub and are used by the packaged example policy (`src/Policies/WebDavPathPolicy.php`).
